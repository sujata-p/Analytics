...
```{r}
library(gbm)
hack.boost_train=gbm(Approved~.,data=hack_train, distribution="gaussian", n.trees=1000,interaction.depth=4)
...
data <- rbind(hack_train, hack_test)
#rm(hack_train,hack_test);gc()
cvFolds <- createFolds(data$Approved[!is.na(data$Approved)], k=5, list=TRUE, returnTrain=FALSE)
varnames <- setdiff(colnames(data), c("IID", "Approved"))
...
test_ids <- data[is.na(Approved),IID]
...
cvFolds <- createFolds(hack_train$Approved[!is.na(hack_train$Approved)], k=5, list=TRUE, returnTrain=FALSE)
varnames <- setdiff(colnames(hack_train), c("IID", "Approved"))
varnames <-c("Monthly_Income", "Existing_EMI", "Loan_Amount", "Loan_Period", "Interest_Rate", "EMI", "Var1", "LCD")
train_sparse <- Matrix(as.matrix(hack_train[!is.na(Approved), varnames, with=F]), sparse=TRUE)
#train_sparse <- Matrix(as.matrix(hack_train[,1:2]), sparse=TRUE)
test_sparse <- Matrix(as.matrix(hack_test[, varnames, with=F]), sparse=TRUE)
y_train <- hack_train[!is.na(Approved),Approved]
test_ids <- hack_test[,IID]
dtrain <- xgb.DMatrix(data=train_sparse, label=y_train)
dtest <- xgb.DMatrix(data=test_sparse)
...
#merged_test <- merge(hack_preds,DT,by.x="IID", by.y="IID", all=TRUE)
...
#sample_submission_test <- fread(paste0(PATH,"sample_submission_v2.csv"), sep=",", na.strings = "", stringsAsFactors=T)
